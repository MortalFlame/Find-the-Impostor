...
let words = JSON.parse(fs.readFileSync(WORDS_FILE, 'utf-8'));

// Capitalize first letter of words and hints
words = words.map(w => ({
    ...
    ws.on('message', (message) => {
        ...
        switch(msg.type) {
            case 'joinLobby':
                ...
                if (!lobbies[lobbyId]) {
                    lobbies[lobbyId] = { 
                        players: [], phase: 'lobby', turnIndex: 0,
            ...
            case 'startGame':
                ...
                broadcast(currentLobby,{ type:'turnUpdate', submissions:[], currentPlayer:lobby.players[lobby.turnIndex].name, phase:lobby.phase });
                break;
            ...
            case 'submitWord':
                ...
                if (lobbySub.turnIndex>=lobbySub.players.length) {
                    if (lobbySub.phase==='round1') {
                        // Start countdown for round2
                        let countdown = 10;
                        const countdownInterval = setInterval(()=>{
                            broadcast(currentLobby, { type:'countdown', countdown });
                            countdown--;
                            if(countdown<0){
                                ...
                                broadcast(currentLobby,{
                                    type:'turnUpdate',
                                    submissions:lobbySub.round1Submissions,
