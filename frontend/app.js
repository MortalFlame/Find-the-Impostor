...
function connect() {
  ...
  ws.onmessage = e => {
    ...
    if (d.type === 'turnUpdate') {
      round1El.innerHTML = d.round1.map(r => `${r.name}: ${r.word}`).join('<br>');
      round2El.innerHTML = d.round2.map(r => `${r.name}: ${r.word}`).join('<br>');
    ...
    if (d.type === 'startVoting') {
      voting.innerHTML = '<h3>Vote</h3>' +
        d.players.map(p => `<button onclick="vote('${p}')">${p}</button>`).join('');
    }
